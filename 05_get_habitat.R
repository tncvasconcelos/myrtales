...
all_point_files <- list.files("myrtales_gbif","cleaned_points.csv", full.names = T)
labels <- gsub(paste0(c("_cleaned_points.csv","myrtales_gbif/"), collapse="|"),"",all_point_files)
all_results <- list()

for(family_index in 1:length(all_point_files)) {
  one_family_points <- fread(all_point_files[family_index])
  one_family_points <- as.data.frame(one_family_points[,c("genus","decimalLatitude","decimalLongitude")])
  #--------------------------------------
  # add information on main biome
  biomes_for_points <- localityToBiome(one_family_points, lat="decimalLatitude",lon="decimalLongitude")
