...
#min_vol$X[which.min(min_vol$Vol)]
...
#  vol <- mean(subset(one_subset$Vol, !is.na(one_subset$Vol)))
#  all_results <- rbind(all_results, cbind(genus,n,vol, all_medians))
#  }
#}
#View(all_results)
all_results <- all_niches
#colnames(all_results) <- colnames(all_niches)
all_results <- all_results[,-c(4:6)]
colnames(all_results)[c(1,2)] <- c("genus","n_points")
all_results$genus <- gsub(".tif","", all_results$genus)


write.csv(all_results, file="datasets/myrtales_niche_summaries.csv",row.names=F)
#View(all_results)
