...
#write.csv(all_myrtales_species[grep("Chamaenerion", all_myrtales_species$genus),], file="Chamaenerion.csv")
...
myrtales_species_total$family <- NA
for(i in 1:nrow(myrtales_species_total)) {
  one_genus <- myrtales_species_total[i,1]
  myrtales_species_total$family[i] <- all_myrtales_species$family[which(all_myrtales_species$genus==one_genus)[1]]
}

#myrtales_species_total <- myrtales_species_total[-1,]
colnames(myrtales_species_total) <- c("genus","n_species","family")
# Merge
